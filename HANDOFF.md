...
**Supabase NOT reseeded from this run (blocked again, 2026-08-31)**: the project host `tgeiancntitgvfcrkvbi.supabase.co` stopped resolving again while `supabase.com`/`github.com` resolve normally -- the project has auto-paused since the successful 2026-08-15 reseed. The live `projects` table therefore still holds the **2,517-row 2026-08-15 seed**, which is now stale in three ways: older model, pre-D14 project types, and pre-D15 amounts. Un-pause it and run step 7; the new seed is 2,393 rows and the script's automatic pruning will clear the ~124+ rows that are no longer in the population.
